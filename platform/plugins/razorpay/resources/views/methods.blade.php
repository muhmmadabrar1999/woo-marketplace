@if (get_payment_setting('status', RAZORPAY_PAYMENT_METHOD_NAME) == 1)
    <li class="list-group-item">
        ...
        <div class="payment_{{ RAZORPAY_PAYMENT_METHOD_NAME }}_wrap payment_collapse_wrap collapse @if ($selecting == RAZORPAY_PAYMENT_METHOD_NAME) show @endif">
            ...
            @php $supportedCurrencies = (new \Woo\Razorpay\Services\Gateways\RazorpayPaymentService)->supportedCurrencyCodes(); @endphp
        ...
        <script>
            $(document).ready(function () {
                ...
                var validatedFormFields = () => {
                    if ($paymentCheckoutForm.find('.shipping_method_input').length &&
                        ! $paymentCheckoutForm.find('.shipping_method_input:checked').val()
                    ) {
                        return false;
                    }
                            ...
                            Stripe.card.createToken(form, function (status, response) {
                                if (response.error) {
                                    if (typeof Woo != 'undefined') {
                                        Woo.showError(response.error.message, _self.data('error-header'));
                                    } else {
