class ShippingManagement {
    init() {
        ...
        $(document).on('keyup', '.input-sync-item', event => {
            ...
            $this.closest('.input-shipping-sync-wrapper').find($this.data('target')).text(Woo.numberFormat(parseFloat(number), 2));
        });
        ...
        $(document).on('keyup', '.base-price-rule-item', event => {
            ...
            $.each($(document).find('.support-shipping .rule-adjustment-price-item'), (index, item) => {
                ...
                $(item).text(Woo.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice)), 2);
            });
        });
