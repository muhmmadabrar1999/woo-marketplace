class Woo {
    constructor() {
        ...
        Woo.initResources();
        Woo.handleCounterUp();
        Woo.initMediaIntegrate();
        if (WooVariables && WooVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = WooVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = WooVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Woo.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Woo.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Woo.showError(data.responseJSON.message);
                } else {
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + WooVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        Woo.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Woo.callScroll($('.list-item-checkbox'));
        }
    ...
    static initMediaIntegrate() {
        ...
        if (jQuery().rvMedia) {
            ...
            Woo.gallerySelectImageTemplate = `<div class="list-photo-hover-overlay">
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Woo.gallerySelectImageTemplate;
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Woo.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Woo.onDragTool);
            $(window).on('mouseup', Woo.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Woo.onDragTool);
        $(window).off('mouseup', Woo.onReleaseTool);
    }
